...
class StudentMemory:
    ...
    def retrieve_long_term(self, user_id: str, thread_id: str, query: str) -> str:
        ...
        context = getattr(user_context, "context", "") or ""
        try:
            facts = self.client.graph.search(user_id=user_id, query=cap_query(query), scope="edges", limit=20)
    ...
    def retrieve_episodic(self, user_id: str, query: str) -> str:
        ...
        results = self.client.graph.search(
            user_id=user_id, query=cap_query(query), scope="episodes", limit=15
        )
